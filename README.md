...
seguintes) portal público de autoagendamento.
...
Integrações externas (Google Calendar, emissão fiscal) ficam sempre atrás de
uma interface (`*Gateway`), com implementação mock disponível para
desenvolvimento e testes, e implementação real plugada apenas quando as
...
no banco por ser o principal identificador de contato do cliente.
...
| Fiscal | Recibo em PDF | Provedor de NFS-e | 6 / 12 |
...
Postgres, deploy via GitHub Actions + SSH — detalhado na Fase 14.
...
multi-barbearia, IA e WhatsApp foram removidos do escopo por decisão do
cliente, e a linha Spring Boot 3.5.x já está fora do período de suporte
open-source.
